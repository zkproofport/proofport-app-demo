...
import { BrowserProvider, Contract, JsonRpcProvider, TypedDataEncoder, Wallet, isAddress, toBeHex, type Eip1193Provider } from 'ethers';
...
import { EXPLORER, GIWA_RPC, TOKEN_ABI, TOKEN_ADDRESS, VAULT_ABI, VAULT_ADDRESS, buildGiwaDepositAction, guardGiwaWalletProvider, parseVaultAmount, vaultErrorMessage, vaultErrorName, waitForVaultReceipt } from './giwa-vault';
...
type Balances = { wallet: bigint | null; deposited: bigint | null; total: bigint; allowance: bigint | null };
...
export function useGiwaVault() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [wallet, deposited, total, allowance] = await timeout(Promise.all([
        address ? token.balanceOf(address) : null, address ? contract.balanceOf(address) : null,
        contract.totalDeposits(), address ? token.allowance(address, VAULT_ADDRESS) : null,
      ]));
  ...
  function reset() {
    ...
    setProof(null); setReceipt(null); setQr(''); setDeepLink(''); setError(''); setPhase('idle'); setTxHash('');
  }
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const changed = () => {
      // Both the proof and transaction belong to the connected account.
      walletVersion.current++; reset(); setAccount(''); setBalances(null);
      setError('Wallet or network changed. Reconnect your wallet and generate a new proof.');
    };
  ...
  async function connect() {
    if (walletBusy.current || locked.current) return;
    ...
    try {
      ...
      if (!accounts[0] || !isAddress(accounts[0])) throw new Error('Select an Ethereum account in your wallet.');
      ...
      if (!latest[0] || !isAddress(latest[0]) || Number(await provider.request({ method: 'eth_chainId' })) !== GIWA_CHAIN_ID || connectedVersion !== walletVersion.current) throw new Error('Reconnect your wallet on GIWA Sepolia.');
  ...
  async function signer(isCurrent: () => boolean) {
    ...
    if (accounts[0]?.toLowerCase() !== account.toLowerCase() || Number(await provider.request({ method: 'eth_chainId' })) !== GIWA_CHAIN_ID) throw new Error('Reconnect the operational wallet on GIWA Sepolia.');
    return new BrowserProvider(guardGiwaWalletProvider(provider, isCurrent)).getSigner(account);
  }

  async function requestProof() {
    if (locked.current || walletBusy.current) return;
    if (!account) { setError('Connect your wallet before generating an eligibility proof.'); return; }
    ...
    try {
      ...
      const image = await client.generateQRCode(pending.deepLink, { width: 1080 });
      if (current !== version.current) return;
      ...
      const bound: BoundProof = { proof: response.proof!, publicInputs: response.publicInputs!.map(value => toBeHex(BigInt(value), 32)), scope, amount: units, account: target };
      setPhase('verifying');
  ...
  async function act(action: 'deposit' | 'approve' | 'withdraw') {
    ...
    } finally { if (isCurrent()) { locked.current = false; } }
  }
  ...
  const approvalNeeded = !!proof && units !== null && (balances?.allowance == null || balances.allowance < units);
